...
Before any search, complete every item of `<kb>/scope.md`. Subjects, dimensions, evidence, lanes, answer shape, and done-criteria all come from the question, and none is invented to fill the template. Revise the frame when a round proves it wrong and explain material changes in `run.md`.

## Rounds

Run at most five research rounds unless the user specifies another cap; record this limit in `scope.md`. Stop earlier when the sufficiency or saturation rule is met, or another limit in `scope.md` is reached.

Sweep: choose at most fifteen distinct research lanes in `scope.md` and spawn one researcher per lane. Each writes one `<run>/round-N/lane-K.jsonl` of candidates with `url`, `title`, `type`, `party`, `subject`, `lane`, and `relevance`, all provisional until the page is opened; `subject` is the thing the page is mainly about, omitted when `scope.md` sets subjects to `none` or the page covers several. Draw lanes from the evidence list in `scope.md`; the README's `type` and `party` values are the checklist of source shapes, and lanes may also follow substitutes, adjacent options, and citation trails. Where subjects publish their own case, cover it and check it against a lane that is not `interested`. When interested sources make consequential claims, seek evidence outside those interests. One lane always seeks criticism, counterevidence, and recent change.
