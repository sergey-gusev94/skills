...
The entire council is strictly read-only: no changes to the project, system state, or external services by you or any voice. The sole exception is the skill's own temp artifacts (the task packet and the script's result and log files). Read-only is enforced by instruction, not by sandboxing — every voice, including the Codex run, has full tool and network access and is trusted to follow it. The deliverable is findings and recommendations only.
...
3. Dispatch all three voices in parallel, in a single block:
   ...
   - Run the script as a background Bash command — foreground commands are capped at ten minutes and council runs often exceed that. Example, with the packet at `/tmp/council-packet-${CLAUDE_SESSION_ID}.md` (include the session id in the name so concurrent sessions cannot clobber each other) and the project under investigation as the second argument (defaults to the current directory):
     `${CLAUDE_SKILL_DIR}/scripts/run-codex-council.sh /tmp/council-packet-${CLAUDE_SESSION_ID}.md /path/to/project`
