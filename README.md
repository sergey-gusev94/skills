...
- The general council and implementation runners default to `gpt-6-astra`; override per run with `CODEX_COUNCIL_MODEL` or `CODEX_IMPLEMENT_MODEL`. Direct Codex skill invocations use the active model except for the required `$lit` and `$topic` child pins.
...
- `$build` is instruction-only and explicitly invoked. Codex delegates one implementation increment at a time, uses five native Codex reviewers, and runs `claude --model fable --effort high -p` for an independent review with one Fable and three Opus subagents. The same review process applies to increments, completed milestones, and the final task result. It requires no hybrid skill runners or council agent definitions.
