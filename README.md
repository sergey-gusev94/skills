...
- `$build` is instruction-only and explicitly invoked. Codex delegates one implementation increment at a time, uses five native Codex reviewers, and runs `claude --model fable --effort high -p` for an independent review with one Fable and two Opus subagents. The same review process applies to increments, completed milestones, and the final task result. It requires no hybrid skill runners or council agent definitions.
