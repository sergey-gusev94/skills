...
- **`hybrid-literature/`** — Claude Code skill (`/lit`): a comprehensive, multi-round literature search and ingest loop that maintains a project-local knowledge base under `literature/`, with a separate ingest-only path for supplied papers.
...
- Literature work uses `gpt-5.6-luna` at maximum reasoning effort for both the ten-subagent council and writer. The council pin ignores `CODEX_COUNCIL_MODEL`; the writer permits the single `CODEX_INGEST_MODEL` override.
