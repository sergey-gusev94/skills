...
1. Establish the review target and baseline from the user's request and repository state, in this order of precedence: a target the user names (reviewed against the baseline they give, or as a standalone audit when none applies); otherwise the uncommitted changes when present; otherwise the current branch's changes against its merge base with the main branch; if none of these yields a target, say there is nothing to review and ask for one. State the target and baseline you chose. Inspect the change before delegating so assignments reflect its actual risk surface. The review is strictly read-only: neither you nor any subagent may modify files, system state, or remote services. The deliverable is findings and recommendations only.
