...
1. Establish the review target and baseline from the user's request and repository state. Unless the user specifies otherwise, review the uncommitted changes when present, otherwise the current branch's changes against its merge base with the main branch, otherwise treat the named target as a standalone audit; state the baseline you chose. Inspect the change before delegating so assignments reflect its actual risk surface. The review is strictly read-only: neither you nor any subagent may modify files, system state, or remote services. The deliverable is findings and recommendations only.
